...
const AjaxComponent = () => {
  ...
  const handleEnviarClick = async () => {
    try {
      ...
      setCodigo(`Código de estado: ${response.status}\nTexto de estado: ${response.statusText}`);
    } catch (error) {
  ...
  return (
    <section>
     <div className='fondo2'>
        ...
        <form>
          URL:
          <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} size="70" />
          <input type="button" value="Mostrar contenidos" onClick={handleEnviarClick} />
        </form>
        <div>
